...
public static class XFunctitect
{
	...
	/// <inheritdoc />
	public static IFunctory Build<TService, TArg>(
		TArg arg,
		string? nextParamName = null
	)
		=> Build(typeof(TService), nextParamName: nextParamName, staticArgs: new object[] { arg! });

	public static IFunctory Build<T>(string? nextParamName = null) => Build(typeof(T), nextParamName: nextParamName);

	/// <inheritdoc />
	public static IFunctory Build(
		Type serviceType,
	...
	private static object? GetService(Type serviceType)
		=> _serviceProvider!.GetService(serviceType) ?? throw new InvalidOperationException($"Service not found for service type {serviceType.Name}");
