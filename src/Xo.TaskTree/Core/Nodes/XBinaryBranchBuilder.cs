...
public class XBinaryBranchBuilder : BaseNodeBuilder, IXBinaryBranchBuilder
{
	...
	protected INodeConfiguration? _TrueConfiguration;
	...
	public virtual IXBinaryBranchBuilder AddTrue<TTrue>(INodeConfiguration? configuration = null)
	{
	...
	public override INode Build()
	{
		...
		IMetaNode metaNode = null!;

		IAsyncFunctory rootFunctory = this._Functitect
			.Build(metaNode.FunctoryType)
			.AsAsync();
	...
	public virtual IXBinaryBranchBuilder AddMetaNode(IMetaNode metaNode)
	{
		this._TrueType = metaNode.
	}
