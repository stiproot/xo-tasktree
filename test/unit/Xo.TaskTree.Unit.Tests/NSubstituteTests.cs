...
public class NSubsistuteTests
{
	...
	{
		// Arrange
		// Behavior: Workflows, with functory factories constructed with the FnFactory, with services mocked using NSubstitute should run.
		...
		var functitect = new FnFactory(provider);
		...
		var n1 = this._nodeFactory.Create()
										.SetContext(context)
										.SetFn(this._functitect.Build(typeof(IY_InStr_OutBool_AsyncService), nameof(Mocked.IY_InStr_OutBool_AsyncService.GetBoolAsync), "flag2").AsAsync())
		...
		var n2 = this._nodeFactory.Create()
										.SetContext(context)
										.SetFn(this._functitect.Build(typeof(IY_InObjBool_OutStr_AsyncService), nameof(Mocked.IY_InObjBool_OutStr_AsyncService.GetStrAsync), null).AsAsync())
